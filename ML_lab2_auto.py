            ...
            for max_iter in max_iters:
                for tol in tols:
                    for verbose in verboses:
                        ...
                        plt.suptitle(("Kmeans, N clusters: ",n_clusters ," n_inits: ",n_inits," max_iter: ",max_iter," tol: ",tol," verbose: ",verbose),fontsize=14, fontweight='bold')
...
def GMM(X,y,max_cluster=None,covariance_types=None,tols=None,max_iters=None,n_inits=None,random_state=None):
    ...
    print("\nThe highest silhouette score is ", best_silhouette, " with ", best_cluster, " clusters")
    print("Best params_/ covariance_types:", best_covariance_type, "/ max_iter:", best_max_iter, "/ tol:", best_tol, "/ n_init:",
          best_n_init,"\n")
    param = "Best params_/ cluster: "+str(best_cluster)+"/ covariance_types:"+ best_covariance_type+ "/ max_iter:"+ str(best_max_iter)+ "/ tol:"+ str(best_tol)+ "/ n_init:"+str(best_n_init)
    ...
    for n_clusters in range_n_clusters:
        for numlocal in numlocals:
            for maxneighbor in maxneighbors:
                ...
                #CLARANS takes too much time so, reduce the number of row
                data = X.iloc[0:500,:]
                ...
                clarans_instance = clarans(data, n_clusters, numlocal, maxneighbor);
            ...
            for metric in metrics:
                for algorithm in algorithms:
                    for leaf_size in leaf_sizes:
                        ...
                        a = np.array([[0 for col in range(len(X.iloc[0,:]))] for row in range(len(set(unique_list)))],dtype=np.float64)
    ...
    for numerical_column,categorical_column in zip(numerical_columns,categorical_columns):
        ...
        for scaler in scalers:
            ...
            for encoder in encoders:
                ...
                silhouette_score,params,kmeans_centerDF_ex = kmeans(x[i][j],y, max_cluster=max_cluster, n_inits=n_inits,max_iters=max_iters,tols=tols,verboses=verboses)
                ...
                silhouette_score, params, GMM_centerDF_ex = GMM(x[i][j],y, max_cluster=max_cluster, covariance_types=covariance_types,tols=tols,max_iters=max_iters,n_inits=n_inits)
                ...
                silhouette_score, params , DBSCAN_centerDF_ex= DBSCANs(x[i][j],y,epsS=epsS,min_samples=min_samples,metrics=metrics,algorithms=algorithms,leaf_sizes=leaf_sizes)
                ...
                silhouette_score, params,MeanShift_centerDF_ex =MeanShifts(x[i][j],y,bandwidths=bandwidths,max_iters=max_iters,n_job=n_job)
